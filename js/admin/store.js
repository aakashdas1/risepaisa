...
// Default admin password hash (SHA-256 of the admin access code)
const DEFAULT_ADMIN_HASH = 'b67028ce9ff7efc94612ed5ce9630f3acf00dc8864c20c3eba6039e3669e9e1e';
  ...
  return {
    ...
    pages: {
      ...
      Advice: {
        ...
        description: 'We provide one-to-one personal advice sessions focused on finance, investing, and practical money management based on your real situation.\n\nThis is a direct video call session where you can discuss your financial goals, problems, and strategy in detail.',
        pricingText: 'Personal advice is conducted via video call.\nIf you need personalized guidance and clear direction, you can book a session and get direct support.',
        priceValue: '2500',
        duration: '2 hours',
        buttonText: 'Contact for Personal Advice'
      }
    }
  };
}
...
function _load() {
  try {
    ...
    if (saved) {
      ...
      // ALWAYS sync courses and resources from source files to fix localStorage cache issue
      _store.courses = _clone(_COURSES);
      _store.resources = _clone(_RESOURCES);
      ...
      if (!_store.settings.navItems.personalAdvice) {
        _store.settings.navItems.personalAdvice = _defaultSettings().navItems.personalAdvice;
      } else if (_store.settings.navItems.personalAdvice.path === '#/contact' || _store.settings.navItems.personalAdvice.path === '#/Advice') {
        // Migration: Fix corrupted path from previous versions
        _store.settings.navItems.personalAdvice.path = '#/consultancy';
      }
      ...
      if (!_store.settings.pages.Advice) _store.settings.pages.Advice = _defaultSettings().pages.Advice;
    } else {
      _store = _seed();
      _persist();
    }
...
// ── Getters ──────────────────────────────────────
export function getCourses()      { return _store.courses.filter(c => c.id !== 1); }
export function getAllCourses()   { return _store.courses; }  // includes hidden courses (for admin/learn pages)
