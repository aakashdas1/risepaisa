...
// ==============================================
import {
  getAllCourses, getResources, getArticles, getSettings,
...
const _E = [42,71,46,29,75,7,30,123,41,24,28];
const _X = [101,43,74,116,56,96,113,23,77,54,50];
  ...
  return `
    ...
    <div class="rp-settings-section">
      ...
      <div class="rp-form-grid">
        <div class="rp-form-group">
          <label class="rp-form-label">Page Title</label>
          <input class="rp-input" id="cp-consultancy-title" value="${_esc(pages.consultancy?.title || '')}" placeholder="Personal Advice">
        </div>
        <div class="rp-form-group">
          <label class="rp-form-label">Button Text</label>
          <input class="rp-input" id="cp-consultancy-btn" value="${_esc(pages.consultancy?.buttonText || '')}" placeholder="Contact for Personal Advice">
        </div>
        <div class="rp-form-group span-2">
      ...
      <div class="rp-form-grid">
        <div class="rp-form-group span-2">
          <div class="rp-toggle-wrap">
            ...
            <span class="rp-toggle-label">Show "Personal Advice" in navbar</span>
          </div>
        </div>
        <div class="rp-form-group">
          <label class="rp-form-label">Menu Label</label>
          <input class="rp-input" id="st-pc-label" value="${_esc(pc.label || 'Personal Advice')}" placeholder="Personal Advice">
        </div>
        <div class="rp-form-group">
...
function _bindSectionEvents() {
  ...
  area.querySelectorAll('[data-course-edit]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      _editMode = 'edit';
      _editIndex = i;
      _editData = JSON.parse(JSON.stringify(getAllCourses()[i]));
      _renderSection();
    });
  });
  ...
  area.querySelectorAll('[data-course-del]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (!confirm(`Delete "${getAllCourses()[i].title}"? This cannot be undone.`)) return;
      const courses = getAllCourses().filter((_, idx) => idx !== i);
      setCourses(courses);
  ...
  area.querySelector('#rp-navbar-save')?.addEventListener('click', () => {
    ...
    s.navItems.personalConsultancy = {
      ...
      label: document.getElementById('st-pc-label')?.value.trim() || 'Personal Advice',
